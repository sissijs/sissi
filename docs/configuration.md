...
Sindie looks for a config file named `.sindie.js` or `.sindie.config.js` in the project root. The file must have a default export that is a function:
...
The function receives a `SindieConfig` instance and may return an object with a `dir` key to override directories.
...
By default Sindie preserves the input path and swaps the file extension (`defaultNaming`). You can switch to directory-based URLs with `directoryNaming`, which turns `about.html` into `about/index.html`:

```js
import { directoryNaming } from 'sindie/naming';
...
Registers a plugin. Plugins follow the same signature as the config function — they receive the `SindieConfig` instance and may return a `dir` override.
...
No-op for Eleventy compatibility. In Sindie, every file whose extension is not in `templateFormats` is passthrough-copied automatically — you never need to opt in explicitly.

## Writing a plugin

A plugin is just a function with the same shape as the config function — it receives the `SindieConfig` instance, calls any config methods it needs, and optionally returns a `dir` override.
...
| Command          | Description                                           |
|------------------|-------------------------------------------------------|
| `sindie build`    | One-time build to the output directory                |
| `sindie watch`    | Watch mode — rebuilds on file changes                 |
| `sindie dev`      | Dev server with watch mode and hot reload             |
